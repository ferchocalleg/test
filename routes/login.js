...
module.exports = function login () {
  ...
  return (req, res, next) => {
    ...
    //models.sequelize.query('SELECT * FROM Users wHERE email = :email AND password = :password AND deletedAt IS NULL', { replacements: { email: req.body.email, password: insecurity.hash(req.body.password)}, model: models.User, plain: true })
    models.sequelize.query('SELECT id,username,email,isAdmin,lastLoginIp,profileImage,totpSecret,isActive,createdAt,updatedAt,deletedAt FROM Users wHERE email = :email AND password = :password AND deletedAt IS NULL', { replacements: { email: req.body.email, password: insecurity.hash(req.body.password)}, model: models.User, plain: true })
